...
# FilesLocations
INPUT_FILE_PATH = "s3://healthcare-etoe-proj-raw-bucket/raw/transactions/FY_2024_SNF_VBP_Facility_Performance.csv"
OUTPUT_DELTA_PATH = "s3://healthcare-etoe-proj-silver-bucket/transactions/FY_2024_SNF_VBP_Facility_Performance/"
...
dynamic_df = gluecontext.create_dynamic_frame.from_options(
    ...
    transformation_ctx="bookmark_ctx_SNF_VBP_FacilityPerform_info" # Unique identifier for tracking this file's state
)
...
if dynamic_df.count() > 0:
    ...
    for col_name in df.columns:
            df = df.withColumn(col_name, trim(col(col_name))) #Performance_Period_FY_2022_Risk_Standardized_Readmission_Rate
            column_name_format = col_name.replace(" ","_").replace(" -- ","_").replace("-","_").replace("(","").replace(")","").replace(":_","_")
    ...
    for col_name in df.columns:
       ...
       cleaned_df = df \
          ...
          .withColumn("Performance_Period_FY_2022_Risk_Standardized_Readmission_Rate", col("Performance_Period_FY_2022_Risk_Standardized_Readmission_Rate").cast("float")) \
